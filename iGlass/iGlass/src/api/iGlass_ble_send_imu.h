...
class iGlass_ble_send_imu : public iGlass_api {
	...
	private:
		void update_ble_imu();
		int update_ble_imu_char(int sensor);
		int update_ble_imu_char_accgyro(int sensor);
		...
		int16_t * imu_buf_arr[NUM_IMU_SENSORS]; // = {acc_buf, gyro_buf, mag_buf};	// ptr to ACC buf, ptr to GYRO buf, ptr to MAG bug
		int imu_buf_idx_arr[NUM_IMU_SENSORS];	// ACC buf idx, GYRO buf idx, MAG buf idx
		int imu_buf_size_arr[NUM_IMU_SENSORS] = {ACC_BUF_SIZE, GYRO_BUF_SIZE, MAG_BUF_SIZE};	// ACC max buf size, GYRO max buf size, MAG max buf size
